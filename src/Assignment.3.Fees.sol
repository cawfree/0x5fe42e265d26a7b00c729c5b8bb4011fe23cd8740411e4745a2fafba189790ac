 ...
 */
contract Assignment3Fees is ReentrancyGuard, ERC20 {
    ...
    constructor(
         ...
         * 
         */
        require(swapFeeBp <= 10_000, "SWAP_FEE_BP");
    ...
    function mint(address recipient, uint256 minLiquidity) external nonReentrant returns (uint256 liquidity) {
        ...
        if (amount0In == 0 || amount1In == 0) revert("INSUFFICIENT_LIQUIDITY");
        ...
        if (liquidity < minLiquidity) revert("INSUFFICIENT_LIQUIDITY");
    ...
    function burn(address recipient) external nonReentrant returns (uint256 amount0Out, uint256 amount1Out) {
        ...
        if (shares == 0) revert("INSUFFICIENT_LIQUIDITY");
